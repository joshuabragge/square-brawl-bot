...
class vJoy(object):
    ...
    def generateJoystickPosition(self,
        # wAxisX = 16000, wAxisY = 16000, lButtons = 0,
        wThrottle = 0, wRudder = 0, wAileron = 0,
        wAxisX = 16000, wAxisY = 16000, wAxisZ = 0, # centered control stick
    ...
    def sendJoystick(self,x, y, button):
        joystickvalue = self.generateJoystickPosition(wAxisX = x,wAxisY = y, lButtons = button)
        return self.update(joystickvalue)

    def aim(self, x=16000, y=16000, button=0, delay=0.05, defaultbutton=0):
        self.sendJoystick(x=x, y=y, button=button)
        time.sleep(delay)
        self.sendJoystick(x=x, y=y, button=defaultbutton)

    #----------Menu/Select--------------------#
    def mselect(self, direction=None, button=0, delay=0.05):
        if direction == None:
